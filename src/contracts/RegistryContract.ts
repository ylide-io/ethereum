...
export class RegistryContract {
	...
	async estimateAndCall(address: string, method: string, args: any[]) {
		const data = this.blockchainController.web3.eth.abi.encodeFunctionCall(
			REGISTRY_ABI.abi.find(t => t.name === method)!,
			args,
		);
		const gasPrice = await this.blockchainController.web3.eth.getGasPrice();
		const gas = await this.blockchainController.web3.eth.estimateGas({
