...
class EthereumWalletController extends sdk_1.AbstractWalletController {
    ...
    async setBonucer(network, from, newBonucer, val) {
        const rc = new contracts_1.RegistryContract(this.writeWeb3, misc_1.EVM_CONTRACTS[network].registry.address);
        return await rc.setBonucer(from, newBonucer, val);
    }
    async setBonuses(network, from, _newcomerBonus, _referrerBonus) {
        const rc = new contracts_1.RegistryContract(this.writeWeb3, misc_1.EVM_CONTRACTS[network].registry.address);
    ...
    async signString(account, message) {
    ...
    async deployRegistryV4(previousContractAddress) {
...
exports.evmWalletFactories = {
    binance: getWalletFactory('binance', async () => !!window.BinanceChain, async () => {
        return new web3_1.default(window.BinanceChain);
    }, async () => {
        return window.BinanceChain;
    }),
    coinbase: getWalletFactory('coinbase', async () => window.ethereum &&
        (window.ethereum.isCoinbaseWallet ||
            (window.ethereum.providers?.length &&
                window.ethereum.providers.find((p) => p.isCoinbaseWallet))), async () => {
        if (window.ethereum.isCoinbaseWallet) {
            return new web3_1.default(window.ethereum);
        }
        else {
            return new web3_1.default(window.ethereum.providers.find((p) => p.isCoinbaseWallet));
        }
    }, async () => {
        if (window.ethereum.isCoinbaseWallet) {
    ...
    trustwallet: getWalletFactory('trustwallet', async () => !!window.trustwallet, async () => new web3_1.default(window.trustwallet), async () => window.trustwallet),
    metamask: getWalletFactory('metamask', async () => window.ethereum &&
        (window.ethereum.isMetaMask ||
            (window.ethereum.providers?.length &&
                window.ethereum.providers.find((p) => p.isMetaMask))), async () => {
        if (window.ethereum.providers?.length) {
            return new web3_1.default(window.ethereum.providers.find((p) => p.isMetaMask));
        }
        else {
            return new web3_1.default(window.ethereum);
        }
