...
class EthereumListSource extends eventemitter3_1.default {
    ...
    async getLast(limit, upToIncluding, mutableParams) {
        if (this.subject.type === sdk_1.BlockchainSourceType.DIRECT) {
            let result;
            if (mutableParams && mutableParams.limit >= limit && mutableParams.messages) {
                const toBlock = mutableParams.toBlock;
                result = await this.reader.advancedRetrieveMessageHistoryByBounds(this.subject.sender, this.subject.recipient, undefined, false, toBlock, undefined, false, limit);
            }
            else {
                result = await this.reader.advancedRetrieveMessageHistoryByBounds(this.subject.sender, this.subject.recipient, undefined, false, undefined, undefined, false, limit);
            }
            if (mutableParams) {
